...
func main() {
	...
	if configPath == "" {
		// Create temporary file if no config file specified
		...
		// Write to specified config file
		if err := os.WriteFile(configPath, otelYAML, 0644); err != nil {
			log.Fatalf("failed to write config to file %s: %v", configPath, err)
		}
	}

	// Set the config file environment variable for the collector
	os.Setenv("OTEL_CONFIG_YAML", configPath)
