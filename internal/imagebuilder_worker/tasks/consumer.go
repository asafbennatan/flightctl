...
// Consume handles incoming jobs from the queue and routes them to appropriate handlers
func (c *Consumer) Consume(ctx context.Context, payload []byte, entryID string, consumer queues.QueueConsumer, log logrus.FieldLogger) error {
	...
	case TaskTypeImageBuild:
		processingErr = c.processImageBuild(ctx, job, log)

	case TaskTypeImageExport:
		processingErr = c.processImageExport(ctx, job, log)
