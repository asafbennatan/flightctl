...
func newTestImageBuildWithTasksService() (ImageBuildWithTasksService, *DummyStore, ImageBuildService, ImageExportService) {
	// Create in-memory SQLite for transaction support
	...
	svc := NewImageBuildWithTasksService(dummyStore, imageBuildSvc, imageExportSvc, log.InitLogs())
	return svc, dummyStore, imageBuildSvc, imageExportSvc
}

func newValidImageBuildWithTasksRequest(buildName, exportName string) api.ImageBuildWithTasksRequest {
	imageBuild := newValidImageBuild(buildName)
	imageExport := newValidImageExport(exportName)

	return api.ImageBuildWithTasksRequest{
		ImageBuild:  imageBuild,
		ImageExport: &imageExport,
	}
}

func TestCreateWithTasksBothResources(t *testing.T) {
	require := require.New(t)
	svc, store, _, _ := newTestImageBuildWithTasksService()
	ctx := context.Background()
	orgId := uuid.New()

	req := newValidImageBuildWithTasksRequest("test-build", "test-export")
...
func TestCreateWithTasksOnlyImageBuild(t *testing.T) {
	require := require.New(t)
	svc, store, _, _ := newTestImageBuildWithTasksService()
	ctx := context.Background()
	orgId := uuid.New()
	...
	req := api.ImageBuildWithTasksRequest{
		ImageBuild:  imageBuild,
		ImageExport: nil,
	}
...
func TestCreateWithTasksImageBuildValidationFails(t *testing.T) {
	require := require.New(t)
	svc, store, _, _ := newTestImageBuildWithTasksService()
	ctx := context.Background()
	orgId := uuid.New()
...
func TestCreateWithTasksImageExportValidationFails(t *testing.T) {
	require := require.New(t)
	svc, _, _, _ := newTestImageBuildWithTasksService()
	ctx := context.Background()
	orgId := uuid.New()
...
func TestCreateWithTasksDuplicateImageBuild(t *testing.T) {
	require := require.New(t)
	svc, store, _, _ := newTestImageBuildWithTasksService()
	ctx := context.Background()
	orgId := uuid.New()
	...
	req := newValidImageBuildWithTasksRequest("duplicate-build", "new-export")
	_, status := svc.Create(ctx, orgId, req)
...
func TestCreateWithTasksDuplicateImageExport(t *testing.T) {
	require := require.New(t)
	svc, store, _, _ := newTestImageBuildWithTasksService()
	ctx := context.Background()
	orgId := uuid.New()
	...
	req := newValidImageBuildWithTasksRequest("new-build", "duplicate-export")
	_, status := svc.Create(ctx, orgId, req)
...
func TestCreateWithTasksImageExportMissingFormats(t *testing.T) {
	require := require.New(t)
	svc, _, _, _ := newTestImageBuildWithTasksService()
	ctx := context.Background()
	orgId := uuid.New()
	...
	imageExport.Spec.Formats = nil // Invalid - formats required
...
func TestCreateWithTasksOverridesExistingSource(t *testing.T) {
	require := require.New(t)
	svc, _, _, _ := newTestImageBuildWithTasksService()
	ctx := context.Background()
	orgId := uuid.New()
