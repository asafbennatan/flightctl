...
func (s *imageExportService) validate(ctx context.Context, orgId uuid.UUID, imageExport *api.ImageExport) []error {
	...
	// Validate formats
	if len(imageExport.Spec.Formats) == 0 {
		errs = append(errs, errors.New("spec.formats is required and must have at least one format"))
	}
	for i, format := range imageExport.Spec.Formats {
		if format.Format == "" {
			errs = append(errs, fmt.Errorf("spec.formats[%d].format is required", i))
		}
	}
