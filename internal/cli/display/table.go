...
func (f *TableFormatter) printImageExportsTable(w *tabwriter.Writer, imageExports ...imagebuilderapi.ImageExport) error {
	f.printHeaderRowLn(w, "NAME", "PHASE", "SOURCE", "OUTPUT", "FORMATS", "AGE")
	for _, ie := range imageExports {
		...
		formats := NoneString
		if len(ie.Spec.Formats) > 0 {
			formatNames := make([]string, 0, len(ie.Spec.Formats))
			for _, exportFmt := range ie.Spec.Formats {
				formatNames = append(formatNames, string(exportFmt.Format))
			}
		...
		f.printTableRowLn(w, name, phase, source, output, formats, age)
	}
	return nil
}
