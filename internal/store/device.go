...
func (s *DeviceStore) PrepareDevicesAfterRestore(ctx context.Context) (int64, error) {
	...
	sql := `
		UPDATE devices 
		SET 
			annotations = COALESCE(annotations, '{}'::jsonb) || jsonb_build_object($1, 'true'),
			status = CASE 
				WHEN status IS NOT NULL THEN 
					(status - 'lastSeen') || jsonb_build_object('summary', jsonb_build_object('status', $2, 'info', $3))
				ELSE 
					jsonb_build_object('summary', jsonb_build_object('status', $2, 'info', $3))
			END
		WHERE deleted_at IS NULL
	`

	result := db.Exec(sql,
		api.DeviceAnnotationWaitingForConnectionAfterRestore,
		api.DeviceSummaryStatusWaitingForConnectionAfterRestore,
