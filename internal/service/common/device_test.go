	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.hasWaitingAnnotation {
				annotations[api.DeviceAnnotationWaitingForConnectionAfterRestore] = tt.waitingAnnotationValue
			}
			...
			if tt.expectedPaused {
				assert.Equal(t, "true", (*device.Metadata.Annotations)[api.DeviceAnnotationPaused], "Paused annotation should be set")
			} else {
				_, exists := (*device.Metadata.Annotations)[api.DeviceAnnotationPaused]
				assert.False(t, exists, "Paused annotation should not be set")
			}

			if tt.expectedWaitingRemoved {
				_, exists := (*device.Metadata.Annotations)[api.DeviceAnnotationWaitingForConnectionAfterRestore]
				assert.False(t, exists, "Waiting annotation should be removed")
			} else if tt.hasWaitingAnnotation {
				assert.Equal(t, tt.waitingAnnotationValue, (*device.Metadata.Annotations)[api.DeviceAnnotationWaitingForConnectionAfterRestore], "Waiting annotation should be preserved")
			}
		})
	}
}
