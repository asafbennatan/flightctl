...
func (g *JWTGenerator) GenerateTokenWithType(identity common.Identity, expiration time.Duration, tokenType string) (string, error) {
	now := time.Now()
...
func (g *JWTGenerator) ValidateToken(tokenString string) (*JWTIdentity, error) {
	...
	if orgs, exists := parsedToken.Get("organizations"); exists {
		if orgList, ok := orgs.([]interface{}); ok {
			orgStrings := make([]string, 0, len(orgList))
			for _, org := range orgList {
				if orgStr, ok := org.(string); ok {
					orgStrings = append(orgStrings, orgStr)
				}
			}
