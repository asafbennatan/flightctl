...
func NewOIDCAuth(oidcAuthority string, clientTlsConfig *tls.Config, orgConfig *common.AuthOrganizationsConfig, usernameClaim string, groupsClaim string, expectedClientId string, scopes []string) (OIDCAuth, error) {
	oidcAuth := OIDCAuth{
...
func (o OIDCAuth) GetAuthConfig() *api.AuthConfig {
	...
	providerName := string(api.AuthProviderInfoTypeOidc)
