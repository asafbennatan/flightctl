...
type MockPAMAuthenticator struct {
	authenticateFunc  func(service, username, password string) error
	lookupUserFunc    func(username string) (*user.User, error)
	getUserGroupsFunc func(systemUser *user.User) ([]string, error)
}

func (m *MockPAMAuthenticator) Authenticate(service, username, password string) error {
	if m.authenticateFunc != nil {
		return m.authenticateFunc(service, username, password)
	}
	return nil
}
...
func NewMockPAMAuthenticator(authenticateFunc func(service, username, password string) error, mockUser *user.User, groups []string) *MockPAMAuthenticator {
...
// Helper function to create a PAM issuer with mock authenticator
func createTestPAMProvider(t *testing.T, mockAuth *MockPAMAuthenticator) *PAMOIDCProvider {
	t.Helper()
	caClient := createTestCAClient(t)
	config := &config.LinuxIssuerAuth{PAMService: "test-service"}
	provider, err := NewPAMOIDCProviderWithAuthenticator(caClient, config, mockAuth)
	require.NoError(t, err)
	return provider
}
...
func TestNewPAMOIDCProvider(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			config:      nil,
			expectError: true,
			errorMsg:    "PAM service must be configured",
		},
		{
			name:        "empty PAM service",
			caClient:    caClient,
			config:      &config.LinuxIssuerAuth{PAMService: ""},
			expectError: true,
			errorMsg:    "PAM service must be configured",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider, err := NewPAMOIDCProvider(tt.caClient, tt.config)
			...
			if tt.expectError {
				...
				assert.Equal(t, "test-service", provider.pamService)
...
func TestPAMOIDCProvider_Token(t *testing.T) {
	...
	caClient := createTestCAClient(t)
	config := &config.LinuxIssuerAuth{PAMService: "test-service"}
	provider, err := NewPAMOIDCProvider(caClient, config)
	require.NoError(t, err)
...
func TestPAMOIDCProvider_GetOpenIDConfiguration(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			config:  &config.LinuxIssuerAuth{PAMService: "test"},
...
func TestPAMOIDCProvider_Integration(t *testing.T) {
	// This test demonstrates the full integration flow using real components
	caClient := createTestCAClient(t)
	config := &config.LinuxIssuerAuth{
		PAMService: "test-service",
		OIDCIssuerAuth: config.OIDCIssuerAuth{
...
func TestPAMOIDCProvider_RealTokenFlow(t *testing.T) {
	// Test the actual PAM issuer token flow with mocked PAM authentication

	t.Run("invalid_credentials", func(t *testing.T) {
		// Mock PAM authentication failure
		mockAuth := NewMockPAMAuthenticator(
			func(service, username, password string) error {
				return errors.New("authentication failed")
			},
			nil, // No user needed for this test
			nil, // No groups needed for this test
		)
		provider := createTestPAMProvider(t, mockAuth)
	...
	t.Run("invalid_refresh_token", func(t *testing.T) {
		...
		mockAuth := NewMockPAMAuthenticator(nil, nil, nil)
		provider := createTestPAMProvider(t, mockAuth)
	...
	t.Run("successful_pam_authentication", func(t *testing.T) {
		// Mock successful PAM authentication with user and groups
		mockUser := &user.User{
			Uid:      "1000",
			Gid:      "1000",
...
func TestPAMOIDCProvider_UserInfoClaims(t *testing.T) {
	...
	// Get a token through the PAM issuer's Token method (real flow)
	tokenReq := &v1alpha1.TokenRequest{
		GrantType: v1alpha1.Password,
		Username:  lo.ToPtr(testUsername),
		Password:  lo.ToPtr(testPassword),
