...
func (c *DeviceNotFoundHTTPClient) cloneRequest(req *http.Request) *http.Request {
	...
	if req.Body != nil {
		// For retries to work with request bodies, the original request should use
		// GetBody function or the body should be seekable. For now, we'll assume
		// the OpenAPI client handles this properly.
