...
// E2E Registry constants
const (
	// E2ERegistryPort is the port used by the local e2e registry
	E2ERegistryPort = "5000"

	// E2ERegistryKindAddress is the default registry address for kind clusters
	// Uses localhost since kind port-forwards the registry
	E2ERegistryKindAddress = "localhost:5000"

	// E2ERegistryEnvVar is the environment variable that can override the registry address
	E2ERegistryEnvVar = "E2E_REGISTRY_ADDRESS"
)

// GetE2ERegistryAddress returns the appropriate registry address for the e2e test environment.
// It checks for environment variable override first, then falls back to kind default.
func (h *Harness) GetE2ERegistryAddress() string {
	// Check for environment variable override
	if addr := os.Getenv(E2ERegistryEnvVar); addr != "" {
		return addr
	}

	// Check if running in kind context
	if h.IsKindCluster() {
		return E2ERegistryKindAddress
	}

	// For non-kind clusters, try to get the ingress domain
	// This would typically be: e2eregistry.<ingress_domain>
	ingressDomain := h.GetIngressDomain()
	if ingressDomain != "" {
		return "e2eregistry." + ingressDomain
	}
...
// IsKindCluster checks if the current kubernetes context is a kind cluster.
func (h *Harness) IsKindCluster() bool {
	kubeContext := os.Getenv("KUBECONTEXT")
	if kubeContext == "" {
		// Try to get from current context
		output, err := h.runKubectlCommand("config", "current-context")
		if err != nil {
			return true // Default to kind if we can't determine
		}
		kubeContext = strings.TrimSpace(output)
	}
	return strings.Contains(kubeContext, "kind")
}

// GetIngressDomain returns the ingress domain for the cluster.
func (h *Harness) GetIngressDomain() string {
	output, err := h.runKubectlCommand("get", "ingresses.config/cluster", "-o", "jsonpath={.spec.domain}")
	if err != nil {
		return ""
	}
...
func (h *Harness) runKubectlCommand(args ...string) (string, error) {
...
func (h *Harness) ResolveImage(registry, imageName, tag string) (ocispec.Descriptor, error) {
	...
	// Use PlainHTTP for local/insecure registries (e2e registry)
