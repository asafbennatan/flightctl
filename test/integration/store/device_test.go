...
var _ = Describe("DeviceStore create", func() {
	...
	Context("Device store", func() {
		...
		It("PrepareDevicesAfterRestore sets annotation, clears lastSeen, and sets status", func() {
			...
			createdDevice, created, err := devStore.CreateOrUpdate(ctx, orgId, testDevice, nil, true, nil, callback)
			Expect(err).ToNot(HaveOccurred())
			...
			Expect(annotations[api.DeviceAnnotationWaitingForConnectionAfterRestore]).To(Equal("true"))
			...
			Expect(device.Status.Summary.Status).To(Equal(api.DeviceSummaryStatusWaitingForConnectionAfterRestore))
		...
		It("PrepareDevicesAfterRestore handles devices with no existing status", func() {
			...
			Expect(updatedDevice.Status.Summary.Status).To(Equal(api.DeviceSummaryStatusWaitingForConnectionAfterRestore))
