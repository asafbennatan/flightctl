...
var _ = Describe("ImageBuildWithTasksService", func() {
	var (
		log           *logrus.Logger
		ctx           context.Context
		orgId         uuid.UUID
		...
		dbName        string
		db            *gorm.DB
		svc           service.ImageBuildWithTasksService
	)
	...
	BeforeEach(func() {
		...
		svc = service.NewImageBuildWithTasksService(storeInst, imageBuildSvc, imageExportSvc, log)
	...
	Context("Rollback on validation failure", func() {
		It("should rollback when ImageExport validation fails", func() {
			...
			invalidExport.Spec.Formats = nil // Invalid - formats required
